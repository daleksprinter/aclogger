...
export class AizuOnlineJudgeClient extends BaseClient {
  ...
  constructor(user: String) {
    ...
    this.user = user;
    this.url =
      "https://judgeapi.u-aizu.ac.jp/submission_records/users/" +
      this.user +
      "?page=0&size=10000";
  }
  fetch() {
  ...
  resToSub(res: any) {
    ...
    const point = null;
    const url =
      "http://judge.u-aizu.ac.jp/onlinejudge/review.jsp?rid=" + res["judgeId"];
...
export class Clients {
  ...
  fetch(callback: any) {
    return Promise.all([
      this.atcoderClient.getAllSubmissions(),
      this.codeforcesClient.getAllSubmissions(),
      this.aojClient.getAllSubmissions(),
      this.yukicoderClient.getAllSubmissions(),
    ]).then((res) => {
      const s = res.reduce(
